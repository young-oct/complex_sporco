# -*- coding: utf-8 -*-
# @Time    : 2020-10-26 8:45 p.m.
# @Author  : young wang
# @FileName: comccmod.py
# @Software: PyCharm
...
"""ADMM algorithms for the complex Convolutional Constrained MOD problem, this script
is modfied from sporco.ccmod based on

J. Kang, D. Hong, J. Liu, G. Baier, N. Yokoya and B. Demir,
"Learning Convolutional Sparse Coding on Complex Domain for
Interferometric Phase Restoration," in
IEEE Transactions on Neural Networks and Learning Systems,
doi: 10.1109/TNNLS.2020.2979546 and
http://brendt.wohlberg.net/publications/garcia-2018-convolutional1.html
...
class ComConvCnstrMODBase(admm.ADMMEqual):
    r"""
...
class ComConvCnstrMOD_IterSM(ComConvCnstrMODBase):
    r"""
    ...
    class Options(ComConvCnstrMODBase.Options):
        ...
        defaults = copy.deepcopy(ComConvCnstrMODBase.Options.defaults)
        ...
        def __init__(self, opt=None):
            ...
            if opt is None:
                opt = {}
            ComConvCnstrMODBase.Options.__init__(self, opt)
    ...
    def __init__(self, Z, S, dsz, opt=None, dimK=1, dimN=2):
        ...
        **Call graph**
        ...
        # Set default options if none specified
        if opt is None:
            opt = ComConvCnstrMOD_IterSM.Options()

        super(ComConvCnstrMODBase, self).__init__(Z, S, dsz, opt, dimK, dimN)
...
class ComConvCnstrMOD_Consensus(admm.ADMMConsensus):
    r"""
    ...
    class Options(admm.ADMMConsensus.Options, ComConvCnstrMODBase.Options):
    ...
    def xistep(self, i):
        ...
        self.X[..., i] = ifftn(self.Xf[..., i], self.cri.Nv,
                                self.cri.axisN)
...
def ComConvCnstrMOD(*args, **kwargs):
    ...
    if method == 'ism':
        base = ComConvCnstrMOD_IterSM
    elif method == 'cns':
        base = ComConvCnstrMOD_Consensus
    else:
        raise ValueError('Unknown ConvCnstrMOD solver method %s' % method)

    # Nested class with dynamically determined inheritance
    class ComConvCnstrMOD(base):
        def __init__(self, *args, **kwargs):
            super(ComConvCnstrMOD, self).__init__(*args, **kwargs)

    # Allow pickling of objects of type ConvCnstrMOD
    _fix_dynamic_class_lookup(ComConvCnstrMOD, method)

    # Return object of the nested class type
    return ComConvCnstrMOD(*args, **kwargs)




def ComConvCnstrMODOptions(opt=None, method='cns'):
    """A wrapper function that dynamically defines a class derived from
    ...
    # Assign base class depending on method selection argument
    if method == 'ism':
        base = ComConvCnstrMOD_IterSM.Options
    elif method == 'cns':
        base = ComConvCnstrMOD_Consensus.Options
    else:
        raise ValueError('Unknown ConvCnstrMOD solver method %s' % method)

    # Nested class with dynamically determined inheritance
    class ComConvCnstrMODOptions(base):
        def __init__(self, opt):
            super(ComConvCnstrMODOptions, self).__init__(opt)

    # Allow pickling of objects of type ConvCnstrMODOptions
    _fix_dynamic_class_lookup(ComConvCnstrMODOptions, method)

    # Return object of the nested class type
    return ComConvCnstrMODOptions(opt)
